...
def get_skill_set(name: str, config: Optional[Union[Dict, SkillSetOptions]] = None) -> List[BaseTool]:
    if name == "slack":
        if not config:
            raise ValueError("Slack skill set requires config")
        return SlackSkillSet(options=SlackSkillSetOptions.model_validate(config)).get_tools()
    else:
