                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ClassDef):
                            ...
                            for child in node.body:
                                if isinstance(child, ast.AnnAssign) and isinstance(child.target, ast.Name) and child.target.id == "description":
                                    ...
                                    if isinstance(child.value, ast.Constant) and isinstance(child.value.value, str):
                                        ...
                                        skill_classes[class_name] = (description, str(file_path))
                                    ...
                                    for target in child.targets:
                                        if isinstance(target, ast.Name) and target.id == "description":
                                            ...
                                            # Handle multi-line string literals
                                                ...
                                                class_name = node.name.lower()
                                                skill_classes[class_name] = (description.strip(), str(file_path))
                except Exception as e:
...
def map_state_to_skill_class(state_name: str, skill_classes: Dict[str, Tuple[str, str]], skill_name: str) -> Optional[str]:
...
def update_states_schema(schema_path: Path) -> None:
    ...
    for state_name in class_states:
        ...
        if state_name not in states_schema["properties"]:
            ...
            if description and ("description" not in state_props or state_props["description"] == default_description):
