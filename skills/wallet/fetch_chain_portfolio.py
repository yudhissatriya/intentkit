...
from skills.wallet.api import fetch_wallet_balances, fetch_token_approvals
...
class FetchChainPortfolioInput(BaseModel):
    ...
    include_approvals: bool = Field(
        default=False,
        description="Whether to include token approvals in the response"
    )
...
class TokenApproval(BaseModel):
    ...
    allowance_formatted: Optional[float] = Field(None, description="Formatted approval amount")
...
class ChainPortfolioOutput(BaseModel):
    ...
    approvals: Optional[List[TokenApproval]] = Field(None, description="Token approvals if requested")
        ...
        **kwargs
        ...
        try:
            ...
            if include_approvals:
                approvals_data = await fetch_token_approvals(self.api_key, address, chain_id)
                ...
                if "error" not in approvals_data:
                    ...
                    for approval in approvals_data.get("result", []):
                        ...
                        is_unlimited = allowance == "115792089237316195423570985008687907853269984665640564039457584007913129639935"
