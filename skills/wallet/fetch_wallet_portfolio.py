...
from skills.wallet.base import WalletBaseTool, CHAIN_MAPPING
...
class FetchWalletPortfolioInput(BaseModel):
    ...
    include_solana: bool = Field(
        default=True,
        description="Whether to include Solana in the analysis"
    )
            ...
            for token in sol_portfolio.get("tokens", []):
                ...
                if token.get("mint"):
                    ...
                    if "error" not in price_result:
                        ...
                        token_balance["usd_value"] = token_balance["balance"] * token_price_usd
