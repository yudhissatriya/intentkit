...
class PredictionData(BaseModel):
    ...
    predictedClass: Optional[str] = Field(
        None,
        description="Predicted direction of APY movement"
    )
    predictedProbability: Optional[float] = Field(
        None,
        description="Probability of the prediction"
    )
    binnedConfidence: Optional[int] = Field(
        None,
...
class PoolData(BaseModel):
    ...
    project: str = Field(
        ...,
        description="Protocol or project name"
    )
    symbol: str = Field(
        ...,
        description="Token or pool symbol"
    )
    tvlUsd: float = Field(
        ...,
        description="Total Value Locked in USD"
    )
    apyBase: Optional[float] = Field(
        None,
        description="Base APY without rewards"
    )
    apyReward: Optional[float] = Field(
        None,
        description="Additional APY from rewards"
    )
    apy: Optional[float] = Field(
        None,
        description="Total APY including rewards"
    )
    rewardTokens: Optional[List[str]] = Field(
        None,
        description="List of reward token addresses"
    )
    pool: Optional[str] = Field(
        None,
        description="Pool identifier"
    )
    apyPct1D: Optional[float] = Field(
        None,
        description="1-day APY percentage change"
    )
    apyPct7D: Optional[float] = Field(
        None,
        description="7-day APY percentage change"
    )
    apyPct30D: Optional[float] = Field(
        None,
        description="30-day APY percentage change"
    )
    stablecoin: bool = Field(
        False,
        description="Whether pool involves stablecoins"
    )
    ilRisk: str = Field(
        "no",
        description="Impermanent loss risk assessment"
    )
    exposure: str = Field(
        "single",
        description="Asset exposure type"
    )
    predictions: Optional[PredictionData] = Field(
        None,
        description="APY movement predictions"
    )
    poolMeta: Optional[str] = Field(
        None,
        description="Additional pool metadata"
    )
    mu: Optional[float] = Field(
        None,
        description="Mean APY value"
    )
    sigma: Optional[float] = Field(
        None,
        description="APY standard deviation"
    )
    ...
    outlier: bool = Field(
        False,
        description="Whether pool is an outlier"
    )
    underlyingTokens: Optional[List[str]] = Field(
        None,
        description="List of underlying token addresses"
    )
    il7d: Optional[float] = Field(
        None,
        description="7-day impermanent loss"
    )
    apyBase7d: Optional[float] = Field(
        None,
        description="7-day base APY"
    )
    apyMean30d: Optional[float] = Field(
        None,
        description="30-day mean APY"
    )
    volumeUsd1d: Optional[float] = Field(
        None,
        description="24h volume in USD"
    )
    volumeUsd7d: Optional[float] = Field(
        None,
        description="7-day volume in USD"
    )
    apyBaseInception: Optional[float] = Field(
        None,
        description="Base APY since inception"
    )
...
class FetchPoolsResponse(BaseModel):
    ...
    data: List[PoolData] = Field(
        default_factory=list,
        description="List of pool data"
    )
