...
from skills.venice_image.input import VeniceImageGenerationInput  # Import the shared input schema
...
class VeniceImageBaseTool(IntentKitSkill):
    ...
    model_id: str = Field(description="The specific model ID used in the Venice API call.")
    # --- Shared Attributes ---
    args_schema: Type[BaseModel] = VeniceImageGenerationInput # Use the shared input schema
    ...
    def get_api_key(self, context: SkillContext) -> Optional[str]:
        ...
        logger.warning(f"No Venice API key found in agent or system config for skill {self.name}")
        return None
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        if not api_key:
            ...
            raise ValueError("Venice AI API key not found. Please configure it in system or agent settings.")
        ...
        if using_agent_key and rate_limit_number and rate_limit_minutes:
            logger.debug(f"Applying agent rate limit ({rate_limit_number}/{rate_limit_minutes} min) for user {context.user_id} on skill {self.name}")
            await self.user_rate_limit_by_category(
             ...
             sys_rate_limit_num = self.skill_store.get_system_config("venice_rate_limit_number", 10) # Example: Default 10
             sys_rate_limit_min = self.skill_store.get_system_config("venice_rate_limit_minutes", 1440) # Example: Default 1 day (1440 min)
             if sys_rate_limit_num and sys_rate_limit_min:
                 logger.debug(f"Applying system rate limit ({sys_rate_limit_num}/{sys_rate_limit_min} min) for user {context.user_id} on skill {self.name}")
                 await self.user_rate_limit_by_category(context.user_id, sys_rate_limit_num, sys_rate_limit_min)
             else:
                 # Fallback if system limits aren't configured at all
                 logger.warning(f"System rate limits for Venice AI not configured. Applying default 10 requests/day for user {context.user_id} on skill {self.name}")
        ...
        try:
            async with httpx.AsyncClient(timeout=180.0) as client: # Set timeout on client
                response = await client.post(
                    api_url, json=payload, headers=headers
                )
                logger.debug(f"Venice API ({self.model_id}) status code: {response.status_code}, Headers: {response.headers}")
                ...
                if response.status_code == 200 and content_type.startswith('image/'):
                    ...
                    if not file_extension: file_extension = 'png' # Default extension

                    key = f"venice/{self.model_id}/{image_hash}.{file_extension}" # e.g., venice/flux-dev/a1b2c3d4e5f6a7b8.png

                    # Store the image bytes
                    stored_url = await store_image_bytes(image_bytes, key, content_type=content_type)
                    logger.info(f"Venice ({self.model_id}) image generated and stored: {stored_url}")
                    return stored_url
                ...
                # Error: Handle non-200 or non-image responses
                    ...
                    try:
                        ...
                        logger.error(f"{error_message} | Failed to parse JSON response: {json_err}")

                    # Raise a more informative error based on status if possible
                    if response.status_code == 400:
                         raise ValueError(f"Bad request to Venice API ({self.model_id}). Check parameters. API response: {response.text}")
                    elif response.status_code == 401:
                         raise PermissionError(f"Authentication failed for Venice API ({self.model_id}). Check API key.")
                    elif response.status_code == 429:
                         raise ConnectionAbortedError(f"Rate limit exceeded for Venice API ({self.model_id}). Try again later.")
                    else:
            ...
            raise Exception(f"Venice API error ({self.model_id}): Status {e.response.status_code} - {e.response.text}") from e
        except httpx.TimeoutException as e:
            logger.error(f"Venice API ({self.model_id}) request timed out: {e}")
            raise TimeoutError(f"The request to Venice AI ({self.model_id}) timed out after 180 seconds.") from e
        except httpx.RequestError as e:
            logger.error(f"Venice API ({self.model_id}) request error: {e}")
            raise ConnectionError(f"Could not connect to Venice API ({self.model_id}): {str(e)}") from e
        except Exception as e:
            logger.error(f"Error generating image with Venice AI ({self.model_id}): {e}", exc_info=True)
            # Avoid leaking internal details unless necessary
            raise Exception(f"An unexpected error occurred while generating the image using model {self.model_id}.") from e