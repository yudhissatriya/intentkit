...
class TextToSpeechInput(BaseModel):
    ...
    text: str = Field(
        description="The text to convert to speech. Required."
    )
...
class TextToSpeech(UnrealSpeechBaseTool):
    ...
    async def _arun(
        ...
        **kwargs
        ...
        api_key = context.config.get("api_key", None) if context and context.config else None
        ...
        if not api_key:
            try:
                ...
                api_key_data = await self.skill_store.get_agent_data(agent_id, "unrealspeech_api_key")
        ...
        if not -1.0 <= speed <= 1.0:
            logger.warning(f"Speed value {speed} is outside valid range (-1.0 to 1.0). Clamping to valid range.")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                return {
                    ...
                    "duration_seconds": result.get("RequestCharacters", 0) // 15,  # Rough estimate (15 chars/sec)
