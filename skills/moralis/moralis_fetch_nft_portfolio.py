...
from skills.wallet.api import fetch_nft_data, get_solana_nfts
...
class FetchNftPortfolioInput(BaseModel):
    ...
    include_solana: bool = Field(
        default=False,
        description="Whether to include Solana NFTs"
    )
    solana_network: str = Field(
        default="mainnet",
        description="Solana network to use (mainnet or devnet)"
    )
    ...
    normalize_metadata: bool = Field(
        True, 
        description="Whether to normalize metadata across different standards"
    )
...
class NftPortfolioOutput(BaseModel):
    ...
    chains: List[str] = Field(default_factory=list, description="Chains included in the response")
        ...
        **kwargs
        ...
        try:
            ...
            if chain_id is not None:
                ...
                from skills.wallet.base import CHAIN_MAPPING
